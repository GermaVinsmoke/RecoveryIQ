...
class SleepEngineTests(unittest.TestCase):
    ...
    def test_energy_windows_shift_under_high_debt(self):
        ...
        self.assertEqual(int((groggy_end - wake).total_seconds() / 60), 90)
        self.assertEqual(target_bedtime, wake + timedelta(hours=16, minutes=29))
    ...
    def test_wake_0757_produces_rise_like_windows(self):
        ...
        self.assertEqual(energy["morning_peak_start"], "2026-03-31T09:50")
        self.assertEqual(energy["morning_peak_end"], "2026-03-31T13:27")
        self.assertEqual(energy["afternoon_dip_start"], "2026-03-31T15:30")
        self.assertEqual(energy["afternoon_dip_end"], "2026-03-31T17:32")
        self.assertEqual(energy["evening_peak_start"], "2026-03-31T19:35")
        self.assertEqual(energy["evening_peak_end"], "2026-03-31T22:21")
        self.assertEqual(energy["target_bedtime"], "2026-04-01T00:26")
        self.assertEqual(energy["wind_down_start"], "2026-03-31T22:41")
        self.assertEqual(energy["melatonin_window_start"], energy["target_bedtime"])
        self.assertEqual(energy["melatonin_window_end"], "2026-04-01T01:26")
        conn.close()

    def test_phase_offsets_match_rise_like_model(self):
        wake = datetime(2026, 1, 1, 8, 24)
        self.assertEqual(wake + timedelta(hours=1, minutes=53), datetime(2026, 1, 1, 10, 17))
        self.assertEqual(wake + timedelta(hours=7, minutes=33), datetime(2026, 1, 1, 15, 57))
        self.assertEqual(wake + timedelta(hours=11, minutes=38), datetime(2026, 1, 1, 20, 2))
        self.assertEqual(wake + timedelta(hours=16, minutes=29), datetime(2026, 1, 2, 0, 53))
